...
class Type1E:
    ...
    def connect(self, ip_master, port_master, ip_plc, port_plc):
        """Connect to PLC

        Args:
            ip_master:          ip address(IPV4) of master
            port_master (int):     port number of master
            ip_plc (str):       ip address(IPV4) to connect PLC
            port_plc (int):     port number of connect PLC
            timeout (float):  timeout second in communication

        """
        self._ip_master = ip_master
        self._port_master = port_master
        self._sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self._sock.bind((ip_master, port_master))
        self._sock.settimeout(self.soc_timeout)
        self.plc_address = (ip_plc, port_plc)
    ...
    def _send(self, send_data):
        ...
        if self._is_connected:
            ...
            self._sock.sendto(send_data, self.plc_address)
        else:
    ...
    def _recv(self):
        ...
        Returns:
            recv_data
        """
        recv_data, _ = self._sock.recvfrom(self._SOCKBUFSIZE)
        return recv_data
